 ...
 * Given an event we returns a list of string times. Each element is a starting of a slot of 30 minutes
...
const getDBWeekEvents = async (date) => {
  ...
  // Order here so we don't need to re-sort in js code
...
const findAvailablitiesFromEvents = (startOfWeek, events) => {
  ...
  // It serves for mapping between the day of the week and the days of the week  
  // For a recurent event, we should consider the day of the week instead of the date
